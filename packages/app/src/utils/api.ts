...
const getBestsellerIsbns = async (): Promise<`${number}`[]> => {
  const url = `${API_ENDPOINT}/bestsellers/isbns`;
  return await (await fetch(url)).json();
};

const getBookByIsbn = async (isbn: string) => {
  const url = `${API_ENDPOINT}/books/by-isbn/${isbn}`;
...
export const getBookById = async (id: string) => {
  const url = `${API_ENDPOINT}/books/by-id/${id}`;
  const json = await (await fetch(url)).json();
...
export const getBooksByKeywordsAndStartIndex = async (
  ...
  const url = `${API_ENDPOINT}/books/by-keywords/${keywords}/${startIndex}`;
  const json = await (await fetch(url)).json();
